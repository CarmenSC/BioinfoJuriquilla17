...
```{r}
print((unique(rawData$doseID)))
print((unique(rawData$concentration)))
#Son esos 38 valores de rawData$concentration
```
...
```{r}
ggplot( rawData, aes( log2(rawData$viability) ) ) +
    geom_histogram(fill = "white", colour="black") +
    facet_wrap(~study) 

ggplot( rawData, aes( (rawData$viability) ) ) +
    geom_histogram(fill = "white", colour="black") +
    facet_wrap(~study)
```
...
issues? Biology?
```{r}
range( rawData$viability )
viability.more.than.zero<-print ( rawData$viability > 0 )
length(viability.more.than.zero)

viability.less.than.zero<-print ( rawData$viability < 100 )
length(viability.more.than.zero)

sum(length(viability.more.than.zero),length(viability.more.than.zero))

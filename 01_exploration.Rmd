...
(Esto es un anexo)Adding an example code: The code below is generating a samole of lnght 100 from a normal distrib with mean 0 and variance 1.
```{r myNewCode}
x<-rnorm(100,0,1)
plot(x)
```
(AQUÍ termina el anexo)
```{r readRaw}
#To install packages
source("https://bioconductor.org/biocLite.R")
#biocLite("devtools")
#biocLite("PharmaPlotter")
#biocLite("ggplot2")
#biocLite("cowplot")
#installed.packages()
...
library(PharmPlotter) #Package de Alejandro para host data
data("rawData", package="PharmPlotter")
pharmacoData <- rawData
pharmacoData
```

What kind of variables are in the data? Are these variables numerical and/or categorical? algunas son numéricas otras son strings. What does each column represent?



```{r quest2}
head( pharmacoData )
str( pharmacoData ) #str structure of object
```
...
tapply( pharmacoData$doseID, pharmacoData$study, function(x){
        length( unique( x ) ) #aplica una operación/función a un subconjunto
    })
```
...
library(ggplot2)
library(cowplot)
#biocLite("cowplot") #biocLite(___) instala cosas de CRAN y bioconductor :)
